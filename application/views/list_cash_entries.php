                                    ...
                                    <div class="card-body">
                                        <table id="datatable-buttons" class="table table-striped dt-responsive nowrap">
                                            ...
                                            <tbody>
                                                ...
                                                $i = 0;
                                                $entry_type = "";
                                                foreach ($entries_data as $entry_row) {
                                                    $i++;
                                                    ?>
                                                    <tr>
                                                        <td><a href="#"><b><?php echo $entry_row->date; ?></b></a></td>
                                                        <td><a href="<?php echo base_url('vendor/entries/' . $entry_row->vendor_hashval); ?>"><?php echo $entry_row->vendor_company ?></a></td>
                                                        <td><?php echo number_format($entry_row->amount,2); ?></td>
                                                        <td><?php echo $entry_row->detail; ?></td>
                                                        <td><?php echo $entry_row->first_name . " " . $entry_row->last_name; ?></td>
                                                        <?php
                                                        if ($entry_row->entry_type == 1) {
                                                            $entry_type = "Debit (بنام )";
                                                        } else {
                                                            $entry_type = "Credit (جمع )";
                                                        }
                                                        ?>
                                                        <td><?php echo $entry_type; ?></td>
                                                        <td><a href="<?php echo base_url('cash/edit_entry/' . $entry_row->entry_hashval); ?>"><i class="dripicons-pencil"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                            <!--<a  href="#" ><img src="<?php echo base_url(SITETHEME . 'myImages/bad.png'); ?>" onclick="delete_entry(<?php echo $entry_row->entry_hashval; ?>)" class="dripicons-trash"></a> -->
                                                        </td>
...
<?php $this->load->view('_footer'); ?>
        ...
        <script>
            function delete_entry(id) {
                Swal.fire({
                    title: 'Are you sure? Do you really want to delete this entry?<br>',
                    text: "You can not change it later! ",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "<?php echo base_url() . 'cash/delete_cash_entry'; ?>",
                            data: {"cash_hashval": id},
                            type: "POST",
